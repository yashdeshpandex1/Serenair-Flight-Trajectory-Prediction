...
def prep_train_data(path='../data/opensky_raw.csv', task='next_instance'):
    ...
    if task=='next_instance':
        save_dir = Path('../data/rnn_data_next_instance/') # define where feature scaler will be saved
        ...
        save_sequences(df_train, df_test, features, target, task='next_instance')
        
        scaler_path = save_dir/'feature_scaler_next_instance.joblib'
        target_path = save_dir/'target_scaler_next_instance.joblib'
        
        joblib.dump(feature_scaler, scaler_path)
        joblib.dump(target_scaler, target_path)
        
    elif task=='next_ten_mins':
        save_dir = Path('../data/rnn_data_next_ten_mins')
        save_dir.mkdir(parents=True, exist_ok=True)
        
        df = initialise_df(path)
        ...
        training_set, testing_set = group_shuffle_split(df=df_engineered)
        df_train, df_test, feature_scaler, target_scaler = scale_dataset(
            training_set, testing_set, features, target
        )
        
        save_sequences(df_train, df_test, features, target, task='next_ten_mins')
        scaler_path = save_dir/'feature_scaler_next_ten_mins.joblib'
        target_path = save_dir/'target_scaler_next_ten_mins.joblib'
...
if __name__ == "__main__":
    ...
    if args.task == 'next_instance':
        filepath = args.file if args.file else '../data/opensky_raw.csv'
        prep_train_data(path=filepath, task='next_instance')
    elif args.task == 'next_ten_mins':
        filepath = args.file if args.file else '../data/opensky_raw.csv'
        prep_train_data(path=filepath, task='next_ten_mins')